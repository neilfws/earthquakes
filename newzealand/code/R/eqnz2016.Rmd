...
```{r get-map}
NZ.map <- get_stamenmap(bbox = c(163.60840,-49.18170,180,-32.28713) , zoom = 6, maptype = "terrain")
p <- ggmap(NZ.map)
```
...
```{r map1}
p <- p  +  stat_density_2d(bins = 20, geom = 'polygon', size = 2, data = geonet, aes(x = longitude, y = latitude, alpha = ..level.., fill = ..level..))
...
```{r map2}
p <- ggmap(NZ.map)
p <- p  +  stat_summary_2d(geom = "tile", bins = 50, data = geonet, aes(x = longitude, y = latitude, z = magnitude), alpha = 0.5)
p <- p + scale_fill_gradient(low = "yellow", high = "red", guide = guide_legend(title = "Magnitude")) + xlab("") + ylab("")
p
```
...
```{r map3}
p <- ggmap(NZ.map)
p <- p + geom_point(data = subset(geonet, magnitude >= 4), aes(x = longitude, y = latitude, color = magnitude), alpha = 0.5) + scale_color_continuous(low = "yellow", high = "red")
p
```
...
ggplot(geonet.rev, aes(origintime, energy.pc)) + geom_line() + theme_bw() + labs(x = "Date/Time", y = "Fraction", title = "New Zealand Earthquakes 2010-2016", subtitle = "Cumulative Energy Release As Fraction of Total Energy Release")
```
...
ggplot(geonet.agg, aes(year, energy.pc)) + geom_bar(stat = "identity", fill = "skyblue3") + theme_bw() + scale_x_continuous(breaks = seq(2010, 2016, 1)) + labs(title = "New Zealand Earthquakes 2010-2016", subtitle = "Cumulative Energy Release As Fraction of Total By Year", x = "Date/Time", y = "Fraction")
```
